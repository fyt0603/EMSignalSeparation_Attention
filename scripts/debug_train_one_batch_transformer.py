"""Transformer 单 batch 训练链路调试脚本。

目的：
- 验证 Dataset/DataLoader、Transformer 前向、SeparationLoss、反向传播与参数更新是否打通。
- 固定同一个 batch 重复训练若干步，观察 loss 是否下降。

运行方式（项目根目录）：
- python -m scripts.debug_train_one_batch_transformer
- python scripts/debug_train_one_batch_transformer.py
"""
