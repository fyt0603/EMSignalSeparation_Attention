...
公式：
- pred_srcA, pred_srcB = apply_complex_mask(pred_mask, mix_spec)
...
class ComplexMaskSeparationLoss(nn.Module):
    """用于复数 mask 双源分离的组合损失。"""
    ...
    def forward(
        ...
        Args:
            pred_mask: 预测 mask，shape [B, 4, F, T]。
            target_mask: 目标 mask，shape [B, 4, F, T]。
        ...
        if pred_mask.ndim != 4 or pred_mask.shape[1] != 4:
            raise ValueError(f"pred_mask must be [B,4,F,T], got {tuple(pred_mask.shape)}")
        if torch.is_complex(pred_mask):
            raise TypeError("pred_mask must be real tensor.")
        if target_mask.shape != pred_mask.shape:
            raise ValueError("target_mask shape must match pred_mask shape.")
        if torch.is_complex(target_mask):
            raise TypeError("target_mask must be real tensor.")
        if mix_spec.ndim != 3:
